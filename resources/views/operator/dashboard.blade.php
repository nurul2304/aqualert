...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // Daftar dummy lokasi sensor
        const sensorLocations = [
            { lat: 1.0964, lng: 104.0406, name: 'Sensor A' },
            { lat: 1.0900, lng: 104.0300, name: 'Sensor B' },
            { lat: 1.1000, lng: 104.0500, name: 'Sensor C' },
            { lat: 1.0925, lng: 104.0450, name: 'Sensor D' },
            { lat: 1.1050, lng: 104.0420, name: 'Sensor E' },
            { lat: 1.0975, lng: 104.0380, name: 'Sensor F' },
        ];

        // Tambahkan semua marker ke map
        sensorLocations.forEach(sensor => {
            L.marker([sensor.lat, sensor.lng]).addTo(map)
                .bindPopup(`<strong>${sensor.name}</strong><br>Lokasi: ${sensor.lat.toFixed(4)}, ${sensor.lng.toFixed(4)}`);
        });

        // Optional: Popup default terbuka di marker pertama
        map.eachLayer(layer => {
        ...
        // Modal logic (jika masih digunakan)
